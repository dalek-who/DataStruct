...
void Push(Stack_ElemType c,Stack *stack);//入栈
Stack_ElemType Pop(Stack* stack);//出栈
Stack_ElemType VisitTop(Stack* stack);//访问栈顶元素(并返回)但不出栈
int IsEmptyStack(Stack* stack);//判断是否为空栈
void CopyStack(Stack* stack, Stack_ElemType s[]);//将整个栈拷贝给一个数组
