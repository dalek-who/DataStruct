...
////////////////////////////////
//改进单向链表的头文件
//改进链表有一个链表，头结点，尾节点，当前指针，当前指针位置，表长
//元素是int型
///////////////////////////////
...
Node* CreatNode(LinkList_ElemType data);//生成具有data==xxxx的节点
void AddNode(Node *to,Node *from,LinkList *list);//将节点nd加入到链表中，p节点的next位置
void HeadInsertNode(LinkList *list, LinkList_ElemType data);//头部插入数据data的节点
void TailInsertNode(LinkList *list, LinkList_ElemType data);//尾部插入数据data的节点
...
void CreatMyList(LinkList* listA,LinkList_ElemType s[], int size);//创建一个链表，链表的数据为s[]1
