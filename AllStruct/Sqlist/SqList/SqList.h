...
int Insert(int i,ElemType elem,SqList *list) ;//在位置i插入元素elem（如果容量不够自动扩容）
void TailInsert(ElemType elem,SqList *list);//在表尾插入
void DisplayList(SqList * list);//display顺序表
int BinSearch(ElemType x, ElemType a[], int size);// 在有序线性表中实现二分查找(返回值有两种模式供选择）


//生成一些测试用的顺序表
//void RandomList(SqList* list);//生成一个随机数据与容量的线性表（数据按非递减排列）
//void CreatTestList(SqList* list);//生成一个元素递增5的等差数列测试用顺序表
void CreatMyList(SqList* list,ElemType myarray[],int array_size);//生成一个我指定的顺序表
