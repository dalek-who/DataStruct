//
// Created by wang on 17-3-11.
//
////////////////////////////////
//改进单向链表的头文件
//改进链表有一个链表，头结点，尾节点，当前指针，当前指针位置，表长
//元素是int型
///////////////////////////////

#ifndef INC_2_19_DELETELEM_LINKLIST_INT_H
#define INC_2_19_DELETELEM_LINKLIST_INT_H

#include <stdio.h>
#include <stdlib.h>
#include <time.h>

#define ERROR 0
#define OK 1

//定义结构
//结点的结构
typedef struct node{
    int data;           //存储的数据
    struct node* next;  //指向下一节点的指针
} Node;

//链表的结构
typedef struct linklist{
    Node *head,*tail,*current;//头结点，尾节点，当前指针。头结点不存储数据，仅用来作为标识.尾指针只想最后一个节点（存有数据）
    int len;//链表长度
    int curpos;//current指针的位置
}LinkList;

//相关的操作
int InitLinkList(LinkList* list);//链表初始化
void ResetCurrent(LinkList *list);//current指针归零（放回头结点）
void DisplayLinkList(LinkList *list);//打印链表
Node* CreatNode(int data);//生成具有data==xxxx的节点
void AddNode(Node *to,Node *from,LinkList *list);//将节点nd加入到链表中，p节点的next位置
void HeadInsertNode(LinkList *list, int data);//头部插入数据data的节点
void TailInsertNode(LinkList *list, int data);//尾部插入数据data的节点
void FreeNode(Node* p);//释放节点内存
void DeleteNextNode(LinkList* list,Node* NowNode);//删除一个节点的next节点。如果删去了尾节点，则把新的最后一个节点变成尾节点

//生成一些特殊的测试用链表
void RandomLinkList(LinkList * list);//随机data与len的链表，data非递减排列
void CreatTestLinkList(LinkList * list);//生成等差数列的链表

#endif //INC_2_19_DELETELEM_LINKLIST_INT_H
