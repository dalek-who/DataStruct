...
typedef struct {
    int key; // 用来排序的关键字
    int another; //用来标识结果是否稳定
}SortData;

typedef SortData  ElemType;
...
Node* RemoveHead(LinkList *list); // 将一个链表的第一个节点摘除下来，并返回指向这个结点的指针
Node* TailInsertNode_ptr(LinkList *list,Node *pnode);//将一个指针指向的节点从尾部插入链表
void ArrayToList(LinkList* listA,ElemType s[], int size);// 把数组转化为链表
